    ...
    <div>
        ...
        <!-- 角色列表区域-->
            ...
            <el-table-column type="expand">
                <template slot-scope="scope">
                    <el-row :class="['bdbottom','vcenter',i1 === 0 ? 'bdtop':'']" v-for="(item1, i1) in scope.row.children" :key='item1.id'  @close="removeRightById(scope.row, item1.id)">
                        <!-- 渲染一级权限 -->
                        <el-col :span='5'>
        ...
        <el-dialog title="分配权限"
            ...
            <span slot="footer" class="dialog-footer" >
                ...
                <el-button type="primary" @click="allorRights">确定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async showSetRightDialog(role) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.errpr('获取权限数据失败')
      }
      //   把获取到的权限数据保存到data中
    ...
    async allorRights() {
      ...
      if (res.meta.status !== 200) {
        return this.$message.errpr('分配权限数据失败')
      }
